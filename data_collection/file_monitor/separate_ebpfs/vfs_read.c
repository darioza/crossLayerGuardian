...
int kprobe__vfs_read(struct pt_regs *ctx, struct file *file, char *buf, size_t count) {
    struct data_t data = {};
    ...
    data.size = count;
    data.offset = file->f_pos;
    data.success = PT_REGS_RC(ctx) >= 0;
    strcpy(data.op_type, "read");
