...
int kprobe__vfs_open(struct pt_regs *ctx, const struct path *path, struct file *file) {
    ...
    data.uid = uid;
    data.fd = file->f_inode->i_ino; // Using inode number as file descriptor equivalent
    data.ts = bpf_ktime_get_ns();
    data.size = 0; // No size involved in open
    data.offset = 0; // No offset involved in open
    data.success = 1; // Opening a file is usually successful if this function is called
    strcpy(data.op_type, "open");

    bpf_probe_read_kernel(&data.fname, sizeof(data.fname), path->dentry->d_name.name);
